...
export default function ProcessSection() {
  ...
  return (
    <section id="process" className="relative min-h-[600px] py-24 md:py-32 overflow-hidden bg-[#0a0a0a]">
          ...
          {(t.process.steps as readonly { title: string; desc: string }[]).map((step, idx) => {
            ...
            return (
              ...
              >
                ...
                <div className="relative bg-white/[0.03] border border-white/10 p-8 h-full backdrop-blur-sm hover:bg-white/[0.06] hover:border-[#C8A45D]/30 transition-all duration-500">
                  <span className="absolute -top-4 -left-2 text-6xl font-serif font-bold text-[#C8A45D]/10 group-hover:text-[#C8A45D]/20 transition-colors duration-500">
                    {stepNums[idx]}
                  </span>

                  <div className="mb-6 relative z-10">
                    <div className="w-14 h-14 rounded-full bg-[#C8A45D]/10 border border-[#C8A45D]/20 flex items-center justify-center group-hover:bg-[#C8A45D]/20 group-hover:border-[#C8A45D]/40 transition-all duration-500">
                      <Icon className="w-6 h-6 text-[#C8A45D]" />
                    </div>
                  </div>
